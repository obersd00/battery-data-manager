...
def plotSpecCapCurves(is_bdms,file,dataset,colors, datasetName,counter, batteryData, pane1, displayLegend, dataSets, fileLabels = None, markersize = 20):
    c = counter % 7
    dataset_label = fileLabels[int(file)]  + ' Cell ' + str(int(dataset + 1)) #'File ' + str(int(file+1)) + ' Cell ' + str(int(dataset + 1))
...
def plotCoulombicEfficiencyCurves(is_bdms,file,dataset,colors, datasetName,counter, batteryData, pane1, displayLegend, dataSets, fileLabels = None, markersize = 20):
    c = counter % 7
    dataset_label = fileLabels[int(file)]  + ' Cell ' + str(int(dataset + 1)) #'File ' + str(int(file+1)) + ' Cell ' + str(int(dataset + 1))
...
def plotMeanVoltageCurves(is_bdms,file,dataset, colors, datasetName, counter, batteryData, pane1, displayLegend, dataSets, fileLabels = None, markersize = 20):
    c = counter % 7
    dataset_label = fileLabels[int(file)]  + ' Cell ' + str(int(dataset + 1)) #'File ' + str(int(file+1)) + ' Cell ' + str(int(dataset) + 1)
    pane1.plot(dataSets[file][dataset].get(datasetName)[0], dataSets[file][dataset].get(datasetName)[1], '.', c=colors[c], label=dataset_label, markersize = markersize)
    counter += 1
...
def plotVoltCurves(is_bdms,combined_bdms_files, file,cycle_numbers, dataset, dataSets,colors, datasetName,counter, batteryData, pane1, displayLegend, fileLabels = None):
    for i in range(len(cycle_numbers)):
        c = counter % 7
        cycle_label = fileLabels[int(file)] + ' Cell ' + str(int(dataset + 1)) + ' Cycle ' + str(cycle_numbers[i])  # generates cycle label as neccessary
        #if is_bdms:
...
def plotdQdVCurves(is_bdms,combined_bdms_files, file, cycle_numbers,dataset, dataSets, colors, datasetName, counter, batteryData, pane1, displayLegend, fileLabels = None):
    for i in range(len(cycle_numbers)):
        # s = counter % 3
        c = counter % 7
        cycle_label = fileLabels[int(file)] + ' Cell ' + str(int(dataset + 1)) + ' Cycle ' + str(cycle_numbers[i])
        #if is_bdms:
