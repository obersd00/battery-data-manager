...
def plotCoulombicEfficiencyCurves(dataset,colors, datasetName,counter, batteryData, pane1, displayLegend, dataSets):
    c = counter % 7
    dataset_label = 'Cell ' + str(int(dataset + 1))
    pane1.plot(dataSets[dataset].get(datasetName)[0], dataSets[dataset].get(datasetName)[3], '.', c=colors[c],
               label=dataset_label)
    displayLegend()
    counter += 1
    return counter
